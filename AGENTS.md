# AGENTS.md — Paymos for CS-Cart

CS-Cart payment method for Paymos stablecoin payments. Thin shell over the bundled Paymos
PHP SDK — the SDK owns signing, webhook verification, dedup and status mapping.
...
- CS-Cart addons are declared by `app/addons/paymos/addon.xml`; settings added to the addon schema require a version bump for existing installs to pick them up.
- Templates live under `design/`; the payment method's own template must stay in sync with the settings the addon declares.
- Order status changes go through `fn_change_order_status` so CS-Cart's own hooks and notifications fire — never write the order row directly.
